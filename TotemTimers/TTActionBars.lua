...
function TTActionBars:new(numbuttons, parent, secondanchor, directionanchor, bartype)
	...
	for i=1,numbuttons do	
        ...
        elseif bartype == "multicast" then
            b.ShowTooltip = TotemTimers.MultiSpellButtonTooltip
        end
        ...
        b:WrapScript(b, "OnDragStart", [[control:CallMethod("StartBarDrag")]])        
        b.StartBarDrag = function(self)
                if not IsModifierKeyDown() and self.bar.alwaysvisible 
                    and not TotemTimers_Settings.Lock and self.bar.secondanchor and not InCombatLockdown() then
        ...
        b:SetScript("OnEvent", function(self)            
                ...
                if spell then
                    local start, duration, enable = GetSpellCooldown(spell)
                        if start and duration then CooldownFrame_SetTimer(self.cooldown, start, duration, enable) end
                    end
                end)  
        if bartype ~= "weapontimer" then 
...
function TTActionBars:SetSpell(nr, spell)
    ...
    if type(spell) == "number" and spell == 0 and self.parent.element then
        local nr = self.parent.element
        TotemTimers.SetEmptyTexCoord(button.icon, nr)
    else
        TotemTimers.SetEmptyTexCoord(button.icon)
    end
    local f = button:GetScript("OnEvent")
    if f then f(button) end
end
...
function TTActionBars:SetSpells(spells)
    for i = 1, #spells do
        self:SetSpell(i,spells[i])
    end
...
function TTActionBars:SetDirection(dir, parentdir, freenotself)
    ...
    if self.alwaysvisible then
        if dir == "up" then
            y = TotemTimers_Settings.TotemMenuSpacing
        elseif dir == "down" then
            y = -TotemTimers_Settings.TotemMenuSpacing
        elseif dir == "left" then
            x = -TotemTimers_Settings.TotemMenuSpacing
        else
            x = TotemTimers_Settings.TotemMenuSpacing
        end
    end
