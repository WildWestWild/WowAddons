...
function XiTimers_UpdateTimers(elapsed) 
	for _,v in pairs(Timers) do
		if v.active then v:update(elapsed) end
	end
end
...
function XiTimers:new(nroftimers, unclickable)
	...
	if unclickable then
        self.button = CreateFrame("Button", "XiTimers_Timer"..XiTimers.numtimers, UIParent, "XiTimersUnsecureTemplate")
    else
        self.button = CreateFrame("Button", "XiTimers_Timer"..XiTimers.numtimers, UIParent, "XiTimersTemplate")
    end
    ...
    self.button.buffCount = _G["XiTimers_Timer"..XiTimers.numtimers.."BuffCount"]
...
function XiTimers:update(elapsed)
		...
		if timers[i] > 0 then
			...
			if timers[i] <= 0 then
				...
				if timers[i]<10 and self.warningMsgs[i] then
					self:Msg(self.warningMsgs[i], "warning", self.MsgIcons[i])
                ...
                if timers[i] <= 10 and timers[i] > 0 then
                    self:Flash(i)
                end                
			end
		end
	end
    if self.nr < 8 and self.timers[1] > 0 and self.timers[1] <= 10 then self.button.time:SetTextColor(1,0,0) end
    ...
    if self.RangeCheck then
        if IsSpellInRange(self.RangeCheck) == 0 then
...
function XiTimers:start(timer, time, duration)
    ...
    if timer == 1 then
        if (self.showCooldown or self.timerOnButton) then
            CooldownFrame_SetTimer(self.button.cooldown, GetTime()-duration+time, duration, 1)
        else
            self.button.cooldown:Hide()
        end
    end
    if self.button.flash[timer] then self.button.flash[timer]:Hide() end
...
function XiTimers:stop(timer)
	...
	if self.timers[timer]>1 then
		if self.earlyExpirationMsgs[timer] then
			self:Msg(self.earlyExpirationMsgs[timer], "expiration", self.MsgIcons[timer])
			self.earlyExpirationMsgs[timer] = nil
		end
	elseif self.expirationMsgs[timer] then
		self:Msg(self.expirationMsgs[timer], "expiration", self.MsgIcons[timer])
    ...
    if not self.dontAlpha then self:SetAlpha(self.button.icons[timer], 0.4) end
    if self.reverseAlpha and InCombatLockdown() then self:SetAlpha(self.button.icons[timer],1) end
	if self.button.flash[timer] then self.button.flash[timer]:Hide() end
...
function XiTimers:SetAlpha(icon, alpha)
    if icon then
        icon:SetAlpha(alpha)
    end
end
...
function XiTimers:Flash(nr)
    if self.dontFlash or (nr > 1 and not self.flashall) or nr > 2 then return end
	local flash = self.FlashRed and self.button.flash[nr] or self.button.icons[nr]
