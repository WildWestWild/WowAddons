...
function TotemTimers.TimerTooltip(self)
    if self.timer and self.element and not IsModifierKeyDown() and TotemTimers_Settings.ShowRaidRangeTooltip then
        ...
        local missingBuffNames, classes = TotemTimers_GetMissingBuffNames(self.element)
        GameTooltip:ClearLines()
        TotemTimers.PositionTooltip(self)
        for name,_ in pairs(missingBuffNames) do
            GameTooltip:AddLine(name, RAID_CLASS_COLORS[classes[name]].r, RAID_CLASS_COLORS[classes[name]].g, RAID_CLASS_COLORS[classes[name]].b)
        end
        GameTooltip:Show()
...
function TotemTimers.WeaponButtonTooltip(self)
    ...
    if s and not self:GetAttribute("doublespell1") then
        text = format(L["Leftclick to cast %s"], s)
