...
function TotemTimers_CreateTrackers()
	...
	shield.activate = function(self)
        ...
        if not TotemTimers_Settings.ShieldTracker then
            self.button:Hide()
        end
      end
   ...
   --earthshield.button.hotkey.SetText = function(self, text) print("SetText: "..tostring(text)) if not text or text == "" then print(debugstack()) end end
	...
	weapon.button:SetScript("OnDragStop", function(self)
            ...
            TotemTimers_ProcessSetting("WeaponBarDirection")
        end)
    weapon.nobars = true
end
...
local function checkESBuff(self)
    ...
    if count then
        ...
        if timeleft-self.timer.timers[1] > 0.1  then
            self.timer:start(1, timeleft, duration)
            self.timer.expirationMsgs[1] = TotemTimers_Settings.EarthShieldMsg and string.format(L["Totem Expired"], name) or nil
            self.timer.earlyExpirationMsgs[1] = TotemTimers_Settings.EarthShieldMsg and string.format(L["Shield removed"], name) or nil
            self.timer.MsgIcons[1] = SpellTextures[SpellIDs.EarthShield]
        end
    elseif self.timer.timers[1] > 0 then
        self.timer:stop(1)
        self.count:SetText("")
...
local function SetWeaponEnchantTextureAndMsg(self, enchant, texture, nr)
    ...
    self.timer.MsgIcons[nr] = texture
    self.timer.expirationMsgs[nr] = TotemTimers_Settings.WeaponMsg and string.format(L["Totem Expired"], enchant) or nil
end
...
function TotemTimers.WeaponEvent(self, event, ...)
	if event == "COMBAT_LOG_EVENT_UNFILTERED" then
		...
		if eventtype == "ENCHANT_APPLIED" and name == playerName and enchantrank then
            ...
            Enchanted = true
			--[[if weapon and weapon == mainHandWeapon then
                TotemTimers_Settings.LastMainEnchants[mainHandWeapon] = {enchant, texture}
                self.icons[1]:SetTexture(texture)
                self.timer.MsgIcons[1] = texture
                self.timer.expirationMsgs[1] = TotemTimers_Settings.WeaponMsg and string.format(TT_EXPIRED, enchant) or nil
			end
			if weapon and weapon == offHandWeapon then
				TotemTimers_Settings.LastOffEnchants[offHandWeapon] = {enchant, texture}
                self.icons[2]:SetTexture(texture)
                self.timer.MsgIcons[2] = texture
                self.timer.expirationMsgs[2] = TotemTimers_Settings.WeaponMsg and string.format(TT_EXPIRED, enchant) or nil
			end]]
		end
    ...
    elseif event == "UNIT_AURA" and select(1,...) == "player" then
        ...
        if TotemTimers_Settings.MaelstromMsg and count == 5 and lastMaelstromCount ~= 5 then
            self.timer:Msg(L["Maelstrom Notifier"], "warning")
        end
