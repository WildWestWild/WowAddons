...
local MultiCastBarParent = nil
local MultiCastBarScripts = {}
...
local defmsgfunc = function(self, warning, type, icon)
    if Sink then
        Sink:Pour(TotemTimers, warning, warningColors[type].color.r,warningColors[type].color.g,warningColors[type].color.b,
...
SettingsFunctions = {
    ShowTimerBars = 
        function(value, Timers) 
            ...
            Timers[17].visibleTimerBars = true
            if Timers[17].timers[1]>0 then
                Timers[17]:ShowTimerBar(n)
            end
        end,
    ...
    AnkhTracker = 
        function(value, Timers)
    		if value and AvailableSpells[SpellNames[SpellIDs.Ankh]] then
    			Timers[5]:activate()
    		else
    			Timers[5]:deactivate()
    		end
    ...
    ShieldTracker =
        function(value, Timers)
    		if (value or TotemTimers_Settings.ActivateHiddenTimers) and AvailableSpells[SpellNames[SpellIDs.LightningShield]] then
    			Timers[6]:activate()
    		else
    			Timers[6]:deactivate()
    		end
    ...
    EarthShieldTracker = 
        function(value, Timers)
    		if (value or TotemTimers_Settings.ActivateHiddenTimers) and AvailableSpells[SpellNames[SpellIDs.EarthShield]] then
    			Timers[7]:activate()
    		else
    			Timers[7]:deactivate()
    		end
    ...
    WeaponTracker =
        function(value, Timers)
    		if (value or TotemTimers_Settings.ActivateHiddenTimers) and AvailableSpells[SpellNames[SpellIDs.RockbiterWeapon]] then
    			Timers[8]:activate()
    		else
    			Timers[8]:deactivate()
    		end
    ...
    EarthShieldLeftButton =
        function(value, Timers)
			if value ~= "recast" then
				Timers[7].button:SetAttribute("*unit1", value)
			else
				TotemTimers_EnableEarthShieldRecast()
			end
        end,
        
    EarthShieldRightButton =
        function(value, Timers)
			if value ~= "recast" then
				Timers[7].button:SetAttribute("*unit2", value)
			else
				TotemTimers_EnableEarthShieldRecast()
			end
        end,
        
    EarthShieldMiddleButton =
        function(value, Timers)
			if value ~= "recast" then
				Timers[7].button:SetAttribute("*unit3", value)
			else
				TotemTimers_EnableEarthShieldRecast()
			end
        end,
    ...
    EarthShieldButton4 =
        function(value, Timers)
			if value ~= "recast" then
				Timers[7].button:SetAttribute("*unit4", value)
			else
				TotemTimers_EnableEarthShieldRecast()
			end
        end,
    ...
    LastWeaponEnchant =  
        function(value, Timers)
            ...
            elseif value == 6 then
                ...
                if value == SpellNames[SpellIDs.RockbiterWeapon] and TotemTimers.AvailableSpellIDs[SpellIDs.WindfuryWeapon] then
                    Timers[8].button:SetAttribute("spell1", SpellNames[SpellIDs.WindfuryWeapon])
                end
            end
        end,
    ...
    LastWeaponEnchant2 =  
        function(value, Timers)
            ...
            if value == SpellNames[SpellIDs.RockbiterWeapon] and TotemTimers.AvailableSpellIDs[SpellIDs.WindfuryWeapon] then
                Timers[8].button:SetAttribute("spell"..b, SpellNames[SpellIDs.WindfuryWeapon])
            end
        end,        
        ...
        function(value, Timers)
            for i = 1,4 do
                if value and not TotemTimers_Settings.MenusAlwaysVisible then 
                    ...
                    Timers[i].button:SetAttribute("*macrotext3", "/script XiTimers.timers["..i.."].earlyExpirationMsgs[1] = nil DestroyTotem("..Timers[i].nr..")")
                    ...
                    Timers[i].button:SetAttribute("*macrotext2", "/script XiTimers.timers["..i.."].earlyExpirationMsgs[1] = nil DestroyTotem("..Timers[i].nr..")")
                end
            end
        end,
    ...
    Show =
        function(value, Timers)
            if value then
                for i=1,4 do
                    if GetTotemInfo(Timers[i].nr) and GetMultiCastTotemSpells(Timers[i].nr) then
                        Timers[i]:activate()
                    end
                end
    ...
    EnhanceCDsSize =
        function(value, Timers)
            ...
            Timers[17]:SetTimeWidth(value*3+10)
    ...
    ActivateHiddenTimers =
        function(value, Timers)
            TotemTimers_ProcessSetting("ShieldTracker")
            TotemTimers_ProcessSetting("EarthShieldTracker")
            TotemTimers_ProcessSetting("WeaponTracker")
        end,
    ...
    WeaponBarDirection = 
        function(value, Timers)
            ...
            if #TTActionBars.bars > 5 then TotemTimers_ProcessSetting("MultiSpellBarDirection") end
        end,
    ...
    MultiSpellBarDirection = 
        function(value, Timers)
            ...
            TTActionBars.bars[6]:SetDirection(v, "free", true)
        end,
        
    HideDefaultTotemBar =
        function(value, Timers)
            if value then
                MultiCastBarParent = MultiCastActionBarFrame:GetParent()
                local frame = CreateFrame("Frame")                
                MultiCastActionBarFrame:SetParent(frame)
                frame:Hide()
                MultiCastActionBarFrame:Hide()
            else
                if MultiCastBarParent then
                    MultiCastActionBarFrame:SetParent(MultiCastBarParent)
                end
            ...
            for i=1,3 do
                if value then
                    if TotemTimers_Settings.MultiSpellReverseBarBinds then
                        getglobal("TT_ActionButton6"..i.."HotKey"):SetText(tostring(10-i))
                        getglobal("TT_ActionButton6"..i):SetAttribute("binding", tostring(10-i))
                    else
                        getglobal("TT_ActionButton6"..i.."HotKey"):SetText(tostring(i))
                        getglobal("TT_ActionButton6"..i):SetAttribute("binding", tostring(i))
                    end
                else
                    getglobal("TT_ActionButton6"..i.."HotKey"):SetText("")
                    getglobal("TT_ActionButton6"..i):SetAttribute("binding", nil)
                end
            end
        end,
    ...
    EnhanceCDsOOCAlpha = 
        function(value, Timers)
            if not InCombatLockdown() then
                for i = 9,15 do
                    Timers[i].button.icons[1]:SetAlpha(value)
                end
                TotemTimers.maelstrom.icon:SetAlpha(value)
                Timers[17].button.icons[1]:SetAlpha(value)
            end
        end,
